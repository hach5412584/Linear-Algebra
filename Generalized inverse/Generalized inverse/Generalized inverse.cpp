...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            if (i == j)
            {
                GS_Array[i].push_back(1);
            }
            else
            {
                GS_Array[i].push_back(0);
            }  
        }
    }
    m = m * 2;
    ...
    while (cow != n)
    {
        ...
        for (int j = 0; j < m; j++)
        {
            ...
            {
                if (cow != n - 1 && GS_Array[cow + 1][j + 1] == 0)
                {
                    for (int s = cow + 1; s < n; s++)
                    {
                        if (GS_Array[s][j + 1] != 0)
                        {
                            CP_Array.assign(GS_Array[cow + 1].begin(), GS_Array[cow + 1].end());
                            GS_Array[cow + 1].assign(GS_Array[s].begin(), GS_Array[s].end());
                            GS_Array[s].assign(CP_Array.begin(), CP_Array.end());
                            end_count = 0;
                            break;
                        }
                        else
